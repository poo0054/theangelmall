					...
					<el-form-item v-else :prop="item.name" :rules="rulesHandle(item)">
						...
						<template v-else-if="item.component=='select'">
							<el-select v-model="form[item.name]" :multiple="item.options.multiple"
									   :placeholder="item.options.placeholder" clearable filterable
									   style="width: 100%;">
						...
						<template v-else-if="item.component=='date'">
							<el-date-picker v-model="form[item.name]" :default-time="item.options.defaultTime"
											:placeholder="item.options.placeholder || '请选择'" :shortcuts="item.options.shortcuts"
