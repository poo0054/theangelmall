				...
				<el-main style="padding:0">
					<el-tabs class="root">
								...
								<div class="sc-filter-main">
									...
									<table v-else>
										...
										<tr v-for="(item,index) in filter" :key="index">
											...
											<td>
												<py-select v-model="item.field" :filter="filter" :options="fields"
														   filterable placeholder="过滤字段"
														   @change="fieldChange(item)">
												</py-select>
											</td>
											<td v-if="showOperator">
												<el-select v-model="item.operator" placeholder="运算符">
													<el-option v-for="ope in item.field.operators || operator"
															   :key="ope.value" :label="ope.label"
															   :value="ope.value"></el-option>
												</el-select>
											</td>
											<td>
												<el-input v-if="!item.field.type" v-model="item.value"
														  disabled placeholder="请选择过滤字段"></el-input>
												<!-- 输入框 -->
												...
												<!-- 日期时间范围 -->
												<el-date-picker v-if="item.field.type=='datetimerange'"
																v-model="item.value" end-placeholder="结束日期"
																start-placeholder="开始日期"
																style="width: 100%;" type="datetimerange"
																value-format="YYYY-MM-DD HH:mm:ss"></el-date-picker>
												<!-- 自定义日期 -->
												<el-date-picker v-if="item.field.type=='customDate'"
																v-model="item.value"
																:placeholder="item.field.placeholder||'请选择'"
												...
												<!-- 标签 -->
												<el-select v-if="item.field.type=='tags'" v-model="item.value" :placeholder="item.field.placeholder||'请输入'"
									...
									<el-button icon="el-icon-plus" text type="primary" @click="addFilter">增加过滤项
									</el-button>
								</div>
							</el-scrollbar>
						</el-tab-pane>
						<el-tab-pane lazy>
							...
							<el-scrollbar>
								<my ref="my" :data="myFilter" :filterName="filterName"
									@selectMyfilter="selectMyfilter"></my>
							</el-scrollbar>
						</el-tab-pane>
					</el-tabs>
				</el-main>
