			...
			<div class="login-form">
				...
				<template v-if="$CONFIG.MY_SHOW_LOGIN_OAUTH">
					...
					<div class="login-oauth">
						...
						<el-button type="success" @click="themallLogin">themall</el-button>
					</div>
				</template>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		themallLogin() {
			let url = `${config.API_URL}` + "/sys/login"
			console.log(url)
		...
		async getCaptcha() {
			if (window.location.href.indexOf("token") !== -1) {
				...
				const expire = this.getQueryVariable("expire");
				if (token) {
					this.$TOOL.cookie.set("TOKEN", token, {
						expires: expire
					})
					//获取用户
					const user = await this.$API.system.user.info.get();
					if (user.code === '00000') {
					...
					//请求权限
					const menu = await this.$API.system.menu.getMenu.get();
					if (menu.code === '00000') {
