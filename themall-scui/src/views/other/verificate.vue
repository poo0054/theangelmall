<template>
	<el-main>
		<el-card header="async-validator内置" shadow="never">
			<el-form ref="form" :model="form" :rules="rules" label-width="100px" status-icon>
				...
				<el-form-item label="类型" prop="type">
					<el-input v-model="form.type"
							  placeholder="类型可为string number boolean array date url email等"></el-input>
				</el-form-item>
				...
				<el-form-item label="异步验证" prop="async">
					<el-input v-model="form.async"
							  placeholder="请输入SCUI最新版本号,form开启status-icon后 可以查看右侧状态"></el-input>
				</el-form-item>
				<el-form-item>
		...
		<el-card header="自定义" shadow="never" style="margin-top: 15px;">
			...
			<el-alert title="自定义验证可在 @/utils/verificate.js 中不断扩展业务需要的验证. 记得要 import 进来"
					  type="success"/>
		</el-card>
	</el-main>
</template>
...
<script>
import {verifyCars, verifyPhone} from '@/utils/verificate'
