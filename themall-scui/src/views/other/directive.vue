<template>
	<el-main>
		<el-card header="v-auth 高精度权限控制" shadow="never">
			...
			<el-button v-auths-all="['list.add','user.add']" type="primary">v-auths-all="['list.add','user.add']"
			</el-button>
		...
		<el-card header="v-role 角色权限控制" shadow="never" style="margin-top: 15px;">
			...
			<el-alert style="margin-top: 20px;"
					  title="v-role指令 是$ROLE的语法糖, 原理是判断是否含有用户所在的角色别名"></el-alert>
		</el-card>
		<el-card header="v-time 时间转换" shadow="never" style="margin-top: 15px;">
			...
			<el-alert style="margin-top: 20px;"
					  title="指令方式日期时间转换,如设置'tip'修饰符将会转换成相对时间,并且每60秒自动更新"></el-alert>
		</el-card>
		<el-card header="v-copy 一键复制" shadow="never" style="margin-top: 15px;">
			...
			<el-alert style="margin-top: 20px;"
					  title="点击复制按钮会将文本框绑定的值复制到剪切板, 试着粘贴到其他地方看看效果"></el-alert>
		</el-card>
	</el-main>
</template>
