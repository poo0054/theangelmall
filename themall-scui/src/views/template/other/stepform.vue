<template>
	<el-main>
		<el-card header="分步表单" shadow="never">
			...
			<el-row>
				<el-col :lg="{span: 8, offset: 8}">
					...
					<el-form v-if="active==1" ref="stepForm_1" :model="form" :rules="rules" label-position="top">
						<el-alert show-icon style="margin-bottom: 15px;" title="确认转账后，资金将直接打入对方账户，无法退回。"
								  type="warning"/>
						...
						<el-form-item label="支付密码" prop="payPassword">
							<el-input v-model="form.payPassword" placeholder="请输入" show-password></el-input>
						</el-form-item>
					</el-form>
					<div v-if="active==2">
						<el-result icon="success" sub-title="预计两小时内到账" title="操作成功">
							<template #extra>
					...
					<el-button v-if="active>0 && active<2" :disabled="submitLoading" @click="pre">上一步</el-button>
					<el-button v-if="active<1" type="primary" @click="next">下一步</el-button>
					<el-button v-if="active==1" :loading="submitLoading" type="primary" @click="submit">提交</el-button>
				</el-col>
			</el-row>
		</el-card>
	</el-main>
