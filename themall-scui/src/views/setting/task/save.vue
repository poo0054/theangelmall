	...
	<el-dialog v-model="visible" :title="titleMap[mode]" :width="400" destroy-on-close @closed="$emit('closed')">
		<el-form ref="dialogForm" :model="form" :rules="rules" label-position="left" label-width="100px">
			<el-form-item label="描述" prop="title">
				<el-input v-model="form.title" clearable placeholder="计划任务标题"></el-input>
			</el-form-item>
			<el-form-item label="执行类" prop="handler">
				<el-input v-model="form.handler" clearable placeholder="计划任务执行类名称"></el-input>
			</el-form-item>
			<el-form-item label="定时规则" prop="cron">
				<sc-cron v-model="form.cron" :shortcuts="shortcuts" clearable
						 placeholder="请输入Cron定时规则"></sc-cron>
			</el-form-item>
