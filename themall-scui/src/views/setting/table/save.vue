<template>
	<el-drawer v-model="visible" :size="1000" :title="titleMap[mode]" destroy-on-close @closed="$emit('closed')">
		<el-container v-loading="loading">
			<el-main style="padding:0 20px 20px 20px">

				<el-form ref="dialogForm" :model="form" :rules="rules" label-position="top" label-width="100px">
					...
					<el-form-item prop="column">
						<sc-form-table v-model="form.column" :addTemplate="addTemplate" drag-sort
									   placeholder="暂无数据">
							...
							<el-table-column align="center" label="隐藏" prop="hide" width="80">
								<template #default="scope">
									<el-checkbox v-model="scope.row.hide"></el-checkbox>
								</template>
							</el-table-column>
							<el-table-column align="center" label="排序" prop="sortable" width="80">
								<template #default="scope">
									<el-checkbox v-model="scope.row.sortable"></el-checkbox>
								</template>
							</el-table-column>
							<el-table-column align="center" label="过滤项" prop="filters" width="90">
								<template #default="scope">
									<el-button :type="scope.row.filters.length==0?'':'primary'" size="small" text
											   @click="setFilters(scope.row.filters)">过滤项
									</el-button>
								</template>
							</el-table-column>
							<el-table-column align="center" label="固定" prop="fixed" width="80">
								<template #default="scope">
			...
			<el-footer>
				<el-button :loading="isSaveing" type="primary" @click="submit">保存</el-button>
				<el-button @click="visible=false">取消</el-button>
			</el-footer>
		</el-container>

		<el-drawer v-model="setFiltersVisible" :size="500" destroy-on-close title="过滤项配置">
			<el-main style="padding:0 20px 20px 20px">
				<sc-form-table v-model="selectionFilters" :addTemplate="filtersAddTemplate" drag-sort
							   placeholder="暂无数据">
