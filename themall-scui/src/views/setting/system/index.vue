			...
			<el-tabs tab-position="top">
				...
				<el-tab-pane label="扩展配置">
					<el-alert style="margin-bottom: 15px;"
							  title="扩展配置为系统业务所有的配置，应该由系统管理员操作，如需用户配置应另起业务配置页面。" type="warning"></el-alert>
					...
					<el-table :data="setting" stripe>
						...
						<el-table-column label="VALUE" prop="value" width="350">
							<template #default="scope">
								<template v-if="scope.row.isSet">
									<el-switch v-if="typeof scope.row.value==='boolean'"
											   v-model="scope.row.value"></el-switch>
						...
						<el-table-column label="CATEGORY" prop="category" width="150">
							<template #default="scope">
								<el-input v-if="scope.row.isSet" v-model="scope.row.category"
										  placeholder="请输入内容"></el-input>
						...
						<el-table-column label="TITLE" prop="title" width="350">
							<template #default="scope">
								<el-input v-if="scope.row.isSet" v-model="scope.row.title"
										  placeholder="请输入内容"></el-input>
							...
							<template #default="scope">
								<el-button-group>
									<el-button size="small" text type="primary"
											   @click="table_edit(scope.row, scope.$index)">{{ scope.row.isSet ? '保存' : "修改" }}
									</el-button>
									<el-button v-if="scope.row.isSet" size="small" text type="primary"
											   @click="scope.row.isSet=false">取消
									</el-button>
					...
					<el-button icon="el-icon-plus" style="margin-top: 20px;" type="primary"
