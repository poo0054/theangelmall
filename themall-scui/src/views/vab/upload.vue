<template>
	<el-main>
		<el-card header="文件示例" shadow="never">
			<sc-upload-file v-model="fileurl" :data="{otherData:'demo'}" :limit="3"
							tip="最多上传3个文件,单个文件不要超过10M,请上传xlsx/docx格式文件">
				<el-button icon="el-icon-upload" type="primary">上传附件</el-button>
			</sc-upload-file>
		</el-card>

		<el-card header="文件示例(值为对象数组,适合保存原始文件名)" shadow="never">
		...
		<el-card header="图片卡片示例(已开启拖拽排序)" shadow="never">
			<sc-upload-multiple v-model="fileurl2" :limit="3" draggable
								tip="最多上传3个文件,单个文件不要超过10M,请上传图像格式文件"></sc-upload-multiple>
		</el-card>

		<el-card header="单图像示例" shadow="never">
			<el-space :size="8" wrap>
				<sc-upload v-model="fileurl3"></sc-upload>
				<sc-upload v-model="fileurl4" icon="el-icon-picture" title="自定义标题"></sc-upload>
				<sc-upload v-model="fileurl5" :apiObj="uploadApi" :on-success="success" :width="220"
				...
				<sc-upload v-model="fileurl6" icon="el-icon-avatar" round title="开启圆形"></sc-upload>
				<sc-upload v-model="fileurl7" :aspectRatio="1/1" :compress="1" :cropper="true"
						   title="开启剪裁"></sc-upload>
			</el-space>
		</el-card>


		<el-card header="在验证表单中使用" shadow="never">
			<el-form ref="ruleForm" :model="form" :rules="rules" label-width="100px">
				...
				<el-form-item label="日期" prop="date">
					<el-date-picker v-model="form.date" placeholder="选择日期" type="date"></el-date-picker>
				</el-form-item>
				<el-form-item>
