...
<template>
		...
		<el-row :gutter="15">
			<el-col :lg="12">
				<el-card shadow="never">
					<el-alert style="margin-bottom:20px;" title="试试右键表格行看看. 支持多级菜单, 动态菜单等等等..."
					...
					<el-table ref="table" :data="tableData" highlight-current-row @row-contextmenu="rowContextmenu">
						...
						<el-table-column label="NAME" prop="name" width="220"></el-table-column>
	...
	<sc-contextmenu ref="contextmenu" @command="handleCommand" @visible-change="visibleChange">
		<sc-contextmenu-item command="a" suffix="Alt+←" title="返回(B)"></sc-contextmenu-item>
		<sc-contextmenu-item command="b" icon="el-icon-refresh" suffix="Ctrl+R"
							 title="重新加载(R)"></sc-contextmenu-item>
		<sc-contextmenu-item divided title="切换兼容性模式">
		...
		<sc-contextmenu-item :disabled="row&&row.state==0" command="d" divided title="属性(P)"></sc-contextmenu-item>
		<sc-contextmenu-item v-if="row&&row.state==0" command="e" title="设置state=1"></sc-contextmenu-item>
	</sc-contextmenu>
